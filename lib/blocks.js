...
var _ = atbcore.deps._;
var pools = require('../pools.json');
var BN = atbcore.crypto.BN;
...
BlockController.prototype.block = function(req, res, next) {
  ...
  if (blockCached) {
    blockCached.confirmations = self.node.services.atbcoind.height - blockCached.height + 1;
    ...
    return next();
      ...
      return async.waterfall([function (callback) {
          ...
          return self.node.services.atbcoind.getBlockHeader(hash, function (err, info) {
        ...
        return self.node.getSubsidy(info.height, function (err, result) {
...
BlockController.prototype._normalizePrevHash = function(hash) {
  // TODO fix atbcore to give back null instead of null hash
...
BlockController.prototype.transformBlock = function(block, info, reward) {
  ...
  var rewardBN = new BigNumber(reward);
  ...
  return {
    ...
    reward: rewardBN.dividedBy(1e8).toString(10),
...
BlockController.prototype.blockIndex = function(req, res) {
  ...
  this.node.services.atbcoind.getBlockHeader(parseInt(height), function(err, info) {
    if (err) {
      return self.common.handleErrors(err, res);
    }
    res.jsonp({
...
BlockController.prototype._getBlockSummary = function(hash, moreTimestamp, next) {
  ...
  if (summaryCache) {
    ...
    self.node.services.atbcoind.getRawBlock(hash, function(err, blockBuffer) {
      if (err) {
        return next(err);
      }

      var br = new atbcore.encoding.BufferReader(blockBuffer);
      ...
      // would require changes in atbcore-node.
      var header = atbcore.BlockHeader.fromBufferReader(br);
      var info = {};
      var txlength = br.readVarintNum();
      info.transactions = [atbcore.Transaction().fromBufferReader(br)];

      self.node.services.atbcoind.getBlockHeader(hash, function(err, blockHeader) {
        if (err) {
          return next(err);
        }
        ...
        var confirmations = self.node.services.atbcoind.height - height + 1;
...
BlockController.prototype.list = function(req, res) {
  ...
  self.node.services.atbcoind.getBlockHashesByTimestamp(lte, gte, function(err, hashes) {
    if(err) {
      return self.common.handleErrors(err, res);
    }
